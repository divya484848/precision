...
var generate = function(docs, res, dirname){
	...
	var row=3;
    ws.cell(1,1,1,10,true).string('Polish report').style({alignment: {horizontal: 'center'}, font: {bold: true, size: 16}});
    ...
    ws.cell(2,4).string("Item").style({font: {bold: true}}).style(box_all);
    ws.cell(2,5).string("Trans date").style({font: {bold: true}}).style(box_all);
    ws.cell(2,6).string("Return type").style({font: {bold: true}}).style(box_all);
    ws.cell(2,7).string("Issue qty").style({font: {bold: true}}).style(box_all);
    ws.cell(2,8).string("Item UoM").style({font: {bold: true}}).style(box_all);
    ws.cell(2,9).string("Issue wt").style({font: {bold: true}}).style(box_all);
    ws.cell(2,10).string("Wt. UoM").style({font: {bold: true}}).style(box_all);
    for(var i=0; i<inv_data.length; i++){
        ...
        ws.cell(row,4).string(inv_data[i].item_name).style(box_all);
		ws.cell(row,5).string(inv_data[i].trans_date).style(box_all);
		ws.cell(row,6).string(inv_data[i].return_type).style(box_all);
        ws.cell(row,7).number(inv_data[i].iss_qty).style(box_all);
        ws.cell(row,8).string((inv_data[i].item_uom)).style(box_all);
        ws.cell(row,9).number(inv_data[i].iss_wt).style(box_all);
        ws.cell(row++,10).string(inv_data[i].wt_uom).style(box_all);
    }

	var filePath = dirname+'/data/polish.xlsx'; 
