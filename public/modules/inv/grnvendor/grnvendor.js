...
app.controller('grnvendorCtrl', function ($scope, $http, $routeParams, $location) {
	var hid = $routeParams.id;
	$scope.trans_id = 0;
	if (hid!=undefined)
		$scope.trans_id = hid;
	$scope.inv_mat_header={trans_type: 'Receipt', return_type: 'Returnable', status: 'Prepared', trans_date: moment().format('YYYY-MM-DD')};
	...
	$scope.retTypeList=['Non-returnable','Returnable'];
	var temp_emp_list=[];
	var temp_dept_list=[];
	var temp_item_list=[];
	var can_save=0;
	...
	$scope.insert_inv_mat_header_record = function() {
		$scope.inv_mat_header = {status: 'Prepared'}
		$scope.inv_mat_header['action'] = 'INSERT_INV_MAT_HEADER';
		hid = undefined;
	...
	$scope.get_op_bal = function() {
		...
		db_inv_mat_details['proc'] = 'inv_mat_headerOps';
		$http.post('/db_data',db_inv_mat_details).success(function(response){
			if (response.data[0][0]['v_cl_qty']==undefined)
				response.data[0][0]['v_cl_qty']=0;
			if (response.data[0][0]['v_cl_wt']==undefined)
				response.data[0][0]['v_cl_wt']=0;
			if (response.error!='None'){
	...
	$scope.update_item_qty = function(){
		$scope.inv_mat_details['cl_qty']=parseInt($scope.inv_mat_details['op_qty'])
						+parseFloat($scope.inv_mat_details['rec_ok_qty']>0 ? $scope.inv_mat_details['rec_ok_qty']: 0)
						+parseFloat($scope.inv_mat_details['rec_rej_qty']>0 ? $scope.inv_mat_details['rec_rej_qty']: 0)
						+parseFloat($scope.inv_mat_details['rec_scrap_qty']>0 ? $scope.inv_mat_details['rec_scrap_qty']: 0)
						+parseFloat($scope.inv_mat_details['rec_unused_qty']>0 ? $scope.inv_mat_details['rec_unused_qty']: 0)
						+parseFloat($scope.inv_mat_details['loss_qty']>0 ? $scope.inv_mat_details['loss_qty']: 0);
		$scope.inv_mat_details_status = '';
	}

	$scope.update_item_wt = function(){
		$scope.inv_mat_details['cl_wt']=$scope.inv_mat_details['op_wt']
						+parseFloat($scope.inv_mat_details['rec_ok_wt']>0 ? $scope.inv_mat_details['rec_ok_wt']: 0)
						+parseFloat($scope.inv_mat_details['rec_rej_wt']>0 ? $scope.inv_mat_details['rec_rej_wt']: 0)
						+parseFloat($scope.inv_mat_details['rec_scrap_wt']>0 ? $scope.inv_mat_details['rec_scrap_wt']: 0)
						+parseFloat($scope.inv_mat_details['rec_unused_wt']>0 ? $scope.inv_mat_details['rec_unused_wt']: 0)
						+parseFloat($scope.inv_mat_details['loss_wt']>0 ? $scope.inv_mat_details['loss_wt']: 0);
		$scope.inv_mat_details_status = '';
	}
})