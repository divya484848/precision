...
app.controller('opbalCtrl', function ($scope, $http, $routeParams, $location) {
	...
	$scope.autoCompleteOptions = {
          /* minimumChars: 1,
           data: function (searchText) {
                ...
                var colors = _.filter(item_list, function (item) {
                    return item.item_name.startsWith(searchText);
                });
	...
	$scope.get_data = function(){
		console.log($scope.inv_op_bal_raw);
	}
	...
	$scope.save_inv_op_bal_raw_record = function() {
		$scope.inv_op_bal_raw['item_id']=$scope.inv_op_bal_raw['item_id'].split(':')[1];
		db_inv_op_bal_raw['trans'] = $scope.inv_op_bal_raw;
		db_inv_op_bal_raw['proc'] = 'inv_op_bal_rawOps';
		$http.post('/db_data',db_inv_op_bal_raw).success(function(response){
			if (response.error!='None'){
