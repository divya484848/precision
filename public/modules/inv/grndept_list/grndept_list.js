...
app.controller('grndept_listCtrl', function ($scope, $http, $routeParams, $location) {
	...
	var select_inv_mat_header_record = function() {
		...
		$http.post('/db_data',db_inv_mat_header).success(function(response){
			if (response.error!='None'){
				...
				$scope.inv_mat_headerList = _.where(response.data[0],{supp_id: null, trans_type: 'Receipt'});
			}
		});
	};
