...
app.controller('grnvendor_listCtrl', function ($scope, $http, $routeParams, $location) {
	...
	var select_inv_mat_header_record = function() {
		...
		$http.post('/db_data',db_inv_mat_header).success(function(response){
			...
			console.log(response.data[0]);
				$scope.inv_mat_headerList = _.where(response.data[0],{emp_id: null, trans_type: 'Receipt'});
			}
		});
	};
