...
app.controller('op_bal_rawCtrl', function ($scope, $http, $routeParams, $location) {
	...
	$scope.save_inv_op_bal_raw_record = function() {
		db_inv_op_bal_raw['trans'] = $scope.inv_op_bal_raw;
		db_inv_op_bal_raw['proc'] = 'inv_op_bal_rawOps';
		if ($scope.inv_op_bal_raw['item_id']==undefined){
			$scope.inv_op_bal_raw_status = 'Item must be selected.';
			return;
		}
		if ($scope.inv_op_bal_raw['op_bal_date']==undefined){
			$scope.inv_op_bal_raw_status = 'Stock date should be mentioned.';
			return;
		}

		if ($scope.inv_op_bal_raw['item_qty'] == undefined && $scope.inv_op_bal_raw['item_wt']==undefined){
			$scope.inv_op_bal_raw_status = 'Either item qty or weight should be present.';
			return;
		}

		if(moment($scope.inv_op_bal_raw['op_bal_date'],'YYYY-MM-DD').isAfter(moment())){
			$scope.inv_op_bal_raw_status = 'Balance date can not be future date.';
			return;
		}
		...
		$http.post('/db_data',db_inv_op_bal_raw).success(function(response){
			if (response.error!='None'){
				...
				select_inv_op_bal_raw_record();
				select_ref_record();
				$scope.inv_op_bal_raw['op_bal_date']= moment().format('YYYY-MM-DD');
			}
		});
	}
