...
app.controller('att_regularCtrl', function ($scope, $http, $routeParams, $location) {
	...
	$scope.save_hrms_att_daily_log_record = function() {
		db_hrms_att_daily_log['trans'] = $scope.hrms_att_daily_log;
		db_hrms_att_daily_log['proc'] = 'hrms_att_daily_logOps';
		$http.post('/db_data',db_hrms_att_daily_log).success(function(response){
			if (response.error!='None'){
				...
				$scope.hrms_att_daily_log_status = response.data[0][0].msg;
				$scope.hrms_att_daily_log = {};
				db_hrms_att_daily_log = {};
				$scope.select_hrms_att_daily_log_record();
			}
		});
	}

	$scope.insert_hrms_att_daily_log_record = function() {
		$scope.hrms_att_daily_log = {status: 'Active'}
		$scope.hrms_att_daily_log['action'] = 'INSERT_HRMS_ATT_DAILY_LOG';
		$scope.hrms_att_daily_log_status = '';
	}

	$scope.update_hrms_att_daily_log_record = function(x) {
		$scope.hrms_att_daily_log = x;
		$scope.hrms_att_daily_log['action'] = 'UPDATE_HRMS_ATT_DAILY_LOG';
		$scope.hrms_att_daily_log_status = '';
	}
