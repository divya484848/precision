...
app.controller('custom_rulesCtrl', function ($scope, $http, $routeParams, $location) {
	...
	var select_admin_ebs_rules_record = function() {
		...
		$http.post('/db_data',db_admin_ebs_rules).success(function(response){
			if (response.error!='None'){
				...
				$scope.admin_ebs_rulesList = _.where(response.data[0],{rule_type:'CUSTOM'})
				if (hid != undefined){
	...
	$scope.insert_admin_ebs_rules_record = function() {
		$scope.admin_ebs_rules = {rule_type: 'CUSTOM', status: 'Active'}
		$scope.admin_ebs_rules['action'] = 'INSERT_ADMIN_EBS_RULES';
		hid = undefined;
