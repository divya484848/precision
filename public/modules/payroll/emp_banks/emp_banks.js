...
app.controller('emp_banksCtrl', function ($scope, $http, $routeParams, $location, $rootScope) {
	...
	var select_payroll_banks_record = function() {
		$scope.payroll_banks['emp_id'] = $rootScope.logged_user_details[0]['sysid'];
		$scope.payroll_banks['status'] = 'Active';
		$scope.payroll_banks['action'] = 'ACC_EXISTS';
		...
		$http.post('/db_data',db_payroll_banks).success(function(response){
			if (response.error!='None'){
				...
				if ($scope.payroll_banksList != undefined){
					if ($scope.payroll_banksList.length > 0) {
						$scope.payroll_banks=response.data[0][0];
						$scope.payroll_banks['action'] = 'UPDATE_PAYROLL_BANKS';
					}
					else {
						$scope.payroll_banks['emp_id'] = $rootScope.logged_user_details[0]['sysid'];
						$scope.payroll_banks['complete_name'] = $rootScope.logged_user_details[0]['complete_name'];
